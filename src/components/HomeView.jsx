...
const HomeView = ({ 
  ...
  const commonIngredients = [
    { name: 'Rice', icon: '🍚' }, { name: 'Beans', icon: '🫘' },
    { name: 'Flour', icon: '🥡' }, { name: 'Sugar', icon: '🍬' },
    { name: 'Beef', icon: '🥩' }, { name: 'Chicken', icon: '🍗' },
    { name: 'Yam', icon: '🍠' }, { name: 'Noodles', icon: '🍜' },
    { name: 'Spaghetti', icon: '🍝' }, { name: 'Banana', icon: '🍌' },
    { name: 'Spinach', icon: '🥬' }, { name: 'Tomato', icon: '🍅' },
    { name: 'Egg', icon: '🥚' }, { name: 'Fish', icon: '🐟' },
  ];
  ...
  const handleQuickAdd = (item) => {
    ...
    if (!pantry.includes(lowerItem)) setPantry(prev => [...prev, lowerItem]);
  };
  ...
  return (
    <div className="flex h-screen bg-[#FDFDFD] overflow-hidden flex-col md:flex-row w-full font-sans">
      ...
      <aside className="w-24 border-r hidden md:flex flex-col items-center py-8 gap-8 bg-white shrink-0 z-20">
        <div title={userName} className="w-12 h-12 bg-orange-500 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg mb-4">
          {getAvatarContent(userName)}
        </div>
        
        <button onClick={() => setActiveTab('all')} className="flex flex-col items-center gap-1 group">
          <LayoutGrid className={activeTab === 'all' ? 'text-orange-500' : 'text-gray-300'} size={22} />
        ...
        <button onClick={() => setActiveTab('market')} className="flex flex-col items-center gap-1 group relative">
          ...
          <span className={`text-[10px] font-bold uppercase ${activeTab === 'market' ? 'text-orange-500' : 'text-gray-300'}`}>Market</span>
        </button>

        <button onClick={() => setActiveTab('tried')} className="flex flex-col items-center gap-1 group">
          <ChefHat className={activeTab === 'tried' ? 'text-orange-500' : 'text-gray-300'} size={22} />
          <span className={`text-[10px] font-bold uppercase ${activeTab === 'tried' ? 'text-orange-500' : 'text-gray-300'}`}>Cooked</span>
        </button>
        
        <button onClick={() => setActiveTab('favorites')} className="flex flex-col items-center gap-1 group">
          <Heart className={activeTab === 'favorites' ? 'text-orange-500' : 'text-gray-300'} size={22} />
          <span className={`text-[10px] font-bold uppercase ${activeTab === 'favorites' ? 'text-orange-500' : 'text-gray-300'}`}>Saved</span>
        </button>
