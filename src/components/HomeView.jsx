...
const HomeView = ({ 
  ...
  const getHeaderSubtext = () => {
    if (activeTab === 'all') return `Find your next meal, ${userName}`;
    if (activeTab === 'pantry') return "Managing your kitchen inventory";
    if (activeTab === 'favorites') return "Managing your favorite recipes";
    if (activeTab === 'tried') return "Managing your tried recipes";
    if (activeTab === 'market') return "Managing your shopping list";
    return `Managing your ${activeTab}`;
  };
  ...
  // --- UPDATED PANTRY TOGGLE (AUTO-NAVIGATE) ---
  const handleTogglePantry = (item) => {
    const lowerItem = item.toLowerCase();
    
    if (pantry.includes(lowerItem)) {
      // Stay on current tab if removing items
      setPantry(pantry.filter(i => i !== lowerItem));
    } else {
      // Add and immediately jump to Discover to show results
        ...
        <div className="fixed inset-0 z-100 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-[40px] p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              <button onClick={() => { setShowLangPicker(true); setShowSettings(false); }} className="w-full p-5 flex items-center justify-between bg-slate-50 rounded-2xl group hover:bg-orange-50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-orange-500 shadow-sm group-hover:bg-orange-500 group-hover:text-white transition-all"><Globe size={20} /></div>
              ...
              <button onClick={() => { setShowShareModal(true); setShowSettings(false); }} className="w-full p-5 flex items-center justify-between bg-slate-50 rounded-2xl group hover:bg-orange-50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-orange-500 shadow-sm group-hover:bg-orange-500 group-hover:text-white transition-all"><Share2 size={20} /></div>
                  <div className="text-left">
              ...
              <button onClick={() => { setShowAbout(true); setShowSettings(false); }} className="w-full p-5 flex items-center justify-between bg-slate-50 rounded-2xl group hover:bg-orange-50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-orange-500 shadow-sm group-hover:bg-orange-500 group-hover:text-white transition-all"><ChefHat size={20} /></div>
                  <div className="text-left">
        ...
        <div className="fixed inset-0 z-110 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-4xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <button onClick={() => { setShowLangPicker(false); setShowSettings(true); }} className="text-slate-400 hover:text-slate-900 flex items-center gap-1 text-[10px] font-black uppercase tracking-widest">Back</button>
...
const MobileNavButton = ({ active, onClick, icon, badge }) => (
  <button onClick={onClick} className={`p-3 relative transition-all rounded-2xl ${active ? 'text-orange-500 bg-orange-50' : 'text-slate-300 active:scale-90'}`}>
    {icon}
    {badge > 0 && <span className="absolute top-2 right-2 bg-orange-600 text-white text-[7px] min-w-3.5 h-3.5 px-1 rounded-full flex items-center justify-center font-black border border-white">{badge}</span>}
  </button>
);
  ...
  return (
    <div className={`bg-white rounded-2xl border overflow-hidden transition-all duration-300 ${isCustom ? 'border-orange-200 bg-orange-50/10' : 'border-slate-100'}`}>
      <button onClick={() => setIsOpen(!isOpen)} className="w-full p-4 flex items-center justify-between group">
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl transition-colors ${isCustom ? 'bg-orange-500 text-white' : 'bg-orange-50/50 group-hover:bg-orange-100/50'}`}>{category.icon}</div>
      ...
      {isOpen && (
        <div className="px-4 pb-5 flex flex-wrap gap-2 animate-in slide-in-from-top-2 duration-300">
          {category.items.map(item => {
            ...
            return (
              <button key={item} onClick={() => onToggle(item)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase border transition-all flex items-center gap-2 ${isSelected ? 'bg-orange-500 text-white border-orange-500 shadow-md shadow-orange-500/20' : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-orange-300'}`}>
                {item}{isCustom && <Trash2 size={10} className={isSelected ? 'text-white/80' : 'text-slate-300'} />}
              </button>
            );
          })}
        </div>
